...
Dashboard 现在已经带登录鉴权，默认所有页面和 API 都需要先登录。

### 默认管理员

在 `dashboard/config.json` 里配置：
...
如果系统第一次启动时还没有账号，会自动创建这个默认管理员。

### 首次强制改密码

- `forceChangeDefaultPassword: true` 时，默认管理员首次登录后必须先修改密码
- 如果你不想要这一步，把它设为 `false`
